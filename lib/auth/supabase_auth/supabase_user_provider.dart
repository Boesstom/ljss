...
class LjssSupabaseUser extends BaseAuthUser {
  LjssSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> ljssSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LjssSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
